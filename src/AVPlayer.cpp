...
void AVPlayer::setPosition(qint64 position)
{
    ...
    qDebug() << "call signal  Q_EMIT positionChanged in func ----->  AVPlayer::setPosition 901";
...
void AVPlayer::timerEvent(QTimerEvent *te)
{
    if (te->timerId() == d->timer_id) {
        ...
        if (d->stop_position_norm == kInvalidPosition) { // or check d->stop_position_norm < 0
            ...
            qDebug() << "call signal  Q_EMIT positionChanged in func -----> AVPlayer::timerEvent 1541";
        ...
        if (t <= d->stop_position_norm) {
            if (!d->seeking) {
                if (isPlaying() && isPaused()) {
                    // qDebug() << "call signal  Q_EMIT positionChanged in func -----> playing 1558" << isPlaying();
                    // qDebug() << "call signal  Q_EMIT positionChanged in func -----> pause 1559" << isPaused();
                } 
                else 
                {
                    qDebug() << "call signal  Q_EMIT positionChanged in func -----> playing 1561" << isPlaying();
                    qDebug() << "call signal  Q_EMIT positionChanged in func -----> pause 1562" << isPaused();
                    Q_EMIT positionChanged(t);
                }
            }
            return;
        ...
        if ((!d->demuxer.atEnd() || d->read_thread->isRunning()) && stopPosition() >= mediaStopPosition()) {
            if (!d->seeking) {
                qDebug() << "call signal  Q_EMIT positionChanged in func -----> AVPlayer::timerEvent 1563";
                Q_EMIT positionChanged(t);
            }
            return;
        }
